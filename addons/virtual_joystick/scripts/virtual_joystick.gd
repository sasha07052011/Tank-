...
signal analogic_chage(move: Vector2, dist: float)
...
func _input(event: InputEvent) -> void:
	if event is InputEventScreenDrag or ( event is InputEventScreenTouch and event.is_pressed()):
		...
		if event_dist_from_center <= boundary * global_scale.x or event.get_index() == ongoing_drag:
			...
			emit_signal("analogic_chage", get_button_pos().normalized(), event_dist_from_center/(joystick.get_rect().size.x))

	if event is InputEventScreenTouch and not event.is_pressed() and event.get_index() == ongoing_drag:
		ongoing_drag = -1
		emit_signal("analogic_chage", Vector2.ZERO, 0)
